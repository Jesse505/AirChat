...
public class Call implements InviteDialogListener {
	...
	public void call(String callee,String calltype) {
		call(callee, null, null, null, null,calltype);						// modified by mandrajg
	}

	/** Starts a new call, inviting a remote user (<i>callee</i>) */
	public void call(String callee, String sdp, String icsi,String calltype) {		// modified by mandrajg
		call(callee, null, null, sdp, icsi,calltype);
	}

	/** Starts a new call, inviting a remote user (<i>callee</i>) */
	public void call(String callee, String from, String contact, String sdp, String icsi,String calltype) {	// modified by mandrajg
		...
		dialog = new InviteDialog(sip_provider, this);
		if (local_sdp != null)
			dialog.invite(callee, from, contact, local_sdp, icsi,calltype);	// modified by mandrajg
		else
			dialog.inviteWithoutOffer(callee, from, contact,calltype);
	}
	...
	/**
	 * request  close incoming/outgoing  video call
	 * 
	 * @author liuyang4 2017.01.09
	 */
	public void audioswitchreq() {
		if (dialog != null) { 
			dialog.doaudioswtich();
		}
	}
	 ...
	 * (e.g. onCallSwitched()).
	 */
	public void onDlgSwitchSuccessResponse(InviteDialog d, int code,
			String reason, Message msg) {
		MyLog.d("SWITCH","onDlgSwitchSuccessResponse!!!");
		if (d != dialog) {
			printLog("NOT the current dialog", LogLevel.HIGH);
			return;
		}
		if (listener != null)
			listener.onCallSwitched(this, msg);
	}
	
	@Override
	public void onDlgSwitchFailureResponse(InviteDialog dialog, int code,
			String reason, Message msg) {
...
//add by liuyang for switch audio call 
	@Override
	public void onDlgSwitch(InviteDialog d, Message msg) {
		// TODO Auto-generated method stub
		if (d != dialog) {
			printLog("NOT the current dialog", LogLevel.HIGH);
			return;
		...
		if (listener != null)
			listener.onCallSwitching(this, msg);
	}

	@Override
	public void onDlgNoAnswer(InviteDialog dialog) {
		if (dialog != this.dialog) {
			printLog("NOT the current dialog", LogLevel.HIGH);
			return;
		}
		if (listener != null)
			listener.onCallNoAnswer(this);
