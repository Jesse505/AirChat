...
public class SipProvider implements Configurable, TransportListener,
	...
	/** SMS UDP transport */
	UdpTransport udp_sms = null;

	/** Tcp server */
	TcpServer tcp_server = null;
	
	/** SMS provider listener */
	SipProviderSMSListener smsListener = null;
	...
	private void startTrasport() {
		...
		host_port = SipStack.default_port; //modify by zyf
		// start udp
		if (transport_udp) {
			try {
				if (host_ipaddr == null)
		...
		try {
			if (host_ipaddr == null)
				udp_sms = new UdpTransport(sms_port, this);
			else
				udp_sms = new UdpTransport(sms_port, host_ipaddr, this);
			sms_port = udp_sms.getPort();
		} catch (Exception e) {
			MyLog.e(TAG, "new UdpTransport failed", e);
		}
	}
		...
		if (transport_udp && proto.equals(PROTO_UDP)) { // UDP
			...
			if (connections == null || !connections.containsKey(conn_id)) { // modified
				...
				try { 
					...
					MyLog.e("SipProvider", "new TcpTransport", e);
					return null;
				}
				addConnection(conn);
			} else {
	...
	public ConnectionIdentifier sendMultiMessage(String msg,String dest_addr) {
		
		try {
			IpAddress dest_ipaddr = IpAddress.getByName(dest_addr);
			return sendMultiMessage(msg, dest_ipaddr);
		} catch (Exception e) {
			printException(e, LogLevel.HIGH);
			return null;
		}
	...
	/**
	 * Sends a Multi Message, Use UDP Protocol
	 */
	private ConnectionIdentifier sendMultiMessage(String msg, IpAddress dest_ipaddr) {
		if (transport_udp ) { // UDP
			try { 
				udp_sms.sendMultiSMS(msg, dest_ipaddr, IPMSGConst.GROUP_PORT);
			} catch (IOException e) {
				printException(e, LogLevel.HIGH);
				return null;
			}
	...
	public ConnectionIdentifier sendMessage(Message msg) {
		...
		MyLog.call_i(TAG + " sendMessage msg:\r\n" + msg.toString());
...
//				}
	...
	public void sendSMS(IPMSGProtocol ipmsgProtocol, String dest_addr){
		try {
			IpAddress dest_ipaddr = IpAddress.getByName(dest_addr);
			sendSMS(ipmsgProtocol,  dest_ipaddr);
		} catch (Exception e) {
			printException(e, LogLevel.HIGH);
		}
	}
	
	public void sendSMS(IPMSGProtocol ipmsgProtocol, IpAddress dest_addr){
		try { // if (ttl>0 && multicast_address) do something?
			udp_sms.sendSMS(ipmsgProtocol, dest_addr, IPMSGConst.PORT);
		} catch (IOException e) {
			printException(e, LogLevel.HIGH);
		}
	}
	
	/** Sends the MutiSMS Message**/
	public void sendMultiSMS(IPMSGProtocol ipmsgProtocol, String dest_addr){
		try {
			IpAddress dest_ipaddr = IpAddress.getByName(dest_addr);
			sendMultiSMS(ipmsgProtocol,  dest_ipaddr);
		} catch (Exception e) {
			printException(e, LogLevel.HIGH);
		}
	}
	
	public void sendMultiSMS(IPMSGProtocol ipmsgProtocol, IpAddress dest_addr){
		try { // if (ttl>0 && multicast_address) do something?
			udp_sms.sendMultiSMS(ipmsgProtocol, dest_addr, IPMSGConst.GROUP_PORT);
		} catch (Exception e) {
			printException(e, LogLevel.HIGH);
		}
	}
	...
	public void stopMultiProvider(String groupIp){
		udp_sms.stopMultiProvider(groupIp);
	}
	...
	protected void processReceivedMessage(Message msg) {
		try { // logs
			...
			//if another invite msg received when callState isn't idle, 486 reponse will be sent;
			if (Receiver.call_state != UserAgent.UA_STATE_IDLE
					&& msg.getRequestLine().getMethod().equals(SipMethods.INVITE)) {
				Message trying100 = MessageFactory.createResponse(msg, 100, SipResponses.reasonOf(100), null);
				sendMessage(trying100);
				MyLog.call_e("SipProvider response 486");
				Message response = MessageFactory.createResponse(msg, 486, SipResponses.reasonOf(486), null);
	...
	public void onReceivedMessage(Transport transport, Message msg) {
		MyLog.call_i("SipProvider onReceivedMessage:\r\n" + msg.toString());
		if (pwl1 == null) {
			pm = (PowerManager) Receiver.mContext.getSystemService(Context.POWER_SERVICE);
			pwl1 = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "Sipdroid.SipProvider.onReceSipMsg");
		}
		pwl1.acquire(); // modified
	...
	@Override
	public void onReceivedMessage(Transport transport, IPMSGProtocol ipmsgProtocol,  String senderIP) {
		if (null != smsListener) {
			if (pwl2 == null) {
				pm = (PowerManager) Receiver.mContext.getSystemService(Context.POWER_SERVICE);
				pwl2 = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "Sipdroid.SipProvider.onReceSmsMsg");
			}
			pwl2.acquire(); // modified
			smsListener.onReceivedSMSdata(ipmsgProtocol, senderIP);
			pwl2.release();
		}
	}
	/** when a new Multi SMS message is received */
	@Override
	public void onReceivedMultiMessage(Transport transport,
			IPMSGProtocol ipmsgProtocol, String senderIP) {
		if (null != smsListener) {
			if (pwl3 == null) {
				pm = (PowerManager) Receiver.mContext.getSystemService(Context.POWER_SERVICE);
				pwl3 = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "Sipdroid.SipProvider.onReceMultiSmsMsg");
			}
			pwl3.acquire(); // modified
			smsListener.onReceivedMultiSMSdata(ipmsgProtocol, senderIP);
			pwl3.release();
		}
		
	}
	
	public void addSMSListener(SipProviderSMSListener smsListener){
