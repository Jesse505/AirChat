...
import org.sipdroid.net.MultiRtpSocket;
...
/**
 * PttRtpStreamReceiver is a generic stream receiver. It receives packets from RTP
 * and writes them into an OutputStream.
 */
public class PttRtpStreamReceiver extends Thread {
...
//	RtpSocket rtp_socket = null;
	MultiRtpSocket rtp_socket = null;

	/** Whether it is running */
	boolean running;
	boolean pauseRece = false;
	...
	/**
	 * Constructs a PttRtpStreamReceiver.
	 * 
	 * @param output_stream
	 *            the stream sink
	 * @param socket
	 ...
	 * @throws IOException 
	 */
	public PttRtpStreamReceiver(SipdroidMultiSocket socket, Codecs.Map payload_type, CallRecorder rec, String groupIP) throws IOException {
		init(socket,groupIP);
		p_type = payload_type;
		call_recorder = rec;
	}

	/** Inits the PttRtpStreamReceiver 
	 * @throws IOException */
	private void init(SipdroidMultiSocket socket, String groupIP) throws IOException {
		if (socket != null){
			rtp_socket = new MultiRtpSocket(socket);
//			rtp_socket.setGroupIp(groupIP);
		}
	}
...
//	}
	...
	public static void adjust(int keyCode,boolean down) {
        AudioManager mAudioManager = (AudioManager) Receiver.mContext.getSystemService(
                Context.AUDIO_SERVICE);
        MyLog.i("jesse", "adjust PttRtpStreamReceiver.speakermode" +PttRtpStreamReceiver.speakermode);
		if (PttRtpStreamReceiver.speakermode == AudioManager.MODE_NORMAL)
		...
		if (!down ^ PttRtpStreamReceiver.speakermode != AudioManager.MODE_NORMAL)
...
//			AudioManager am = (AudioManager) Receiver.mContext.getSystemService(Context.AUDIO_SERVICE);
	...
	public static void setMode(int mode) {
		...
		if (Integer.parseInt(Build.VERSION.SDK) >= 5) {
			...
			if (samsung) PttRtpStreamSender.changed = true;
		} else
			am.setMode(mode);
	}
...
//		}
	...
	public static void restoreSettings() {
		if (PreferenceManager.getDefaultSharedPreferences(Receiver.mContext).getBoolean(org.sipdroid.sipua.ui.Settings.PREF_OLDVALID, org.sipdroid.sipua.ui.Settings.DEFAULT_OLDVALID)) {
			...
			PowerManager.WakeLock wl = pm.newWakeLock(PowerManager.SCREEN_BRIGHT_WAKE_LOCK |
					PowerManager.ACQUIRE_CAUSES_WAKEUP, "Sipdroid.PttRtpStreamReceiver");
			wl.acquire(1000);
		}
		restoreMode();
	}
	...
	public void run() {
		...
		while (running) {
			...
			try {
//				MyLog.i("ptt", "rtp_socket receive >>>>>>>>>>>>> start " +
//						"\n SourceAddress:" + rtp_socket.getSourceAddress());
				rtp_socket.receive(rtp_packet);
//				MyLog.i("ptt", "rtp_socket receive >>>>>>>>>>>>> end");
				if (rtp_socket.getSourceAddress().getHostAddress().equals(SessionUtils.getLocalIPaddress())) {
					Receiver.onPttState(UserAgent.PTT_STATE_SPEAKING,null);
					continue;
				}else {
					MyLog.i("ptt", "PTT_STATE_OCCUPY");
					Receiver.onPttState(UserAgent.PTT_STATE_OCCUPY,rtp_socket.getSourceAddress().getHostAddress());
				}
				if (timeout != 0) {
					tg.stopTone();
					track.pause();
				...
				if (Receiver.ptt_state != UserAgent.PTT_STATE_OFF) {
					Receiver.onPttState(UserAgent.PTT_STATE_FREE,null);
				}
				
//				MyLog.e("ptt", "rtp_socket.receive", e);
				if (timeout == 0 && nodata) {
					tg.startTone(ToneGenerator.TONE_SUP_RINGTONE);
				}
				rtp_socket.getDatagramSocket().disconnect();
				//modify by zyf 两分钟没有人讲话，自动取消ptt会话
				if (++timeout > 120) {
					if (Receiver.ptt_launcher.equals(SessionUtils.getLocalIPaddress())) {
						Receiver.engine(Receiver.mContext).dismissPttSession(null);
					}else {
						Receiver.engine(Receiver.mContext).closePttSession();
					}
					break;
				}
			}
			...
			if (running && timeout == 0) {		
				 ...
				 if (seq != 0) {
					 getseq = gseq&0xff;
					 expseq = ++seq&0xff;
					 if (m == PttRtpStreamSender.m) vm = m;
...
//						 Receiver.ptt_state == UserAgent.PTT_STATE_FREE ||
//						 Receiver.ptt_state == UserAgent.PTT_STATE_OCCUPY)) {
...
//		cleanupBluetooth();
	...
	private static void println(String str) {
		if (!Sipdroid.release) System.out.println("PttRtpStreamReceiver: " + str);
	}
