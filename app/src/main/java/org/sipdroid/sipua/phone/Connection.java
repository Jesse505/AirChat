...
public class Connection
{
    ...
    private SqlDBOperate mDBOperate;//add by lss
  ...
  //add by lss for declear callist type (video or audio)
    ...
    public void log(long start)
	{
	    ...
	    // get the callerinfo object and then log the call with it.
	    {
	        ...
	        Date curDate = new Date(System.currentTimeMillis());//get now time   
	        String dateNow = formatter.format(curDate);
	        mDBOperate = new SqlDBOperate(Receiver.mContext); 
	        if (number.indexOf("<sip:") >= 0 && number.indexOf(">") >= 0)
	        	number = number.substring(number.indexOf("<sip:")+5,number.indexOf(">"));
	            
	        mDBOperate.addCallInfo(number,callLogType, dateNow, (int) duration / 1000,mDBOperate.getReceiverIpByReceiverImei(number),callType);//modify by lss for declear callist type (video or audio)
	        MyLog.d("lss","Connection dateNow="+dateNow+":::duration"+duration+":::getAddress()"+mDBOperate.getReceiverIpByReceiverImei(number)
	        		+":::callType="+callType);
	        //add end
	    }
	}
