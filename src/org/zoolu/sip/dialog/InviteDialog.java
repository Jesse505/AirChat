...
public class InviteDialog extends Dialog implements TransactionClientListener,
	...
	/** The SWITCH TransactionServer. */ //add by liuyang
	TransactionServer switch_ts;
	...
	protected static final int D_SWITCHING = 21; //add by liuyang 
	...
	public void invite(String callee, String caller, String contact,
			String session_descriptor, String icsi,String calltype) {							// modified by mandrajg
		...
		Message invite = MessageFactory.createInviteRequest(sip_provider,
				request_uri, to_url, from_url, contact_url, session_descriptor, icsi,calltype);	// modified by liuyang
		// do invite
		invite(invite);
	}
	 ...
	 */
	public void inviteWithoutOffer(String callee, String caller, String contact,String type) {
		invite_offer = false;
		invite(callee, caller, contact, null, null,type);		// modified by liuyang
	}
	...
	// private void respond(Message resp)
	{
		...
		}//add by liuyang for video call switch
		if (method.equals(SipMethods.SWITCH)) {

			if (!verifyStatus(statusIs(D_SWITCHING)))
				return;

			changeStatus(D_CALL);
			switch_ts.respondWith(resp);
		}
	}
	...
	/**
	 * if no one pick up the call or refuse the call in 35s, this method will be called
	 */
	public void noAnswer() {
		refuse(408, SipResponses.reasonOf(408));
		listener.onDlgNoAnswer(this);
	}
	...
	/**
	 * switch the call. This method should be called when the switch call type 
	 * TransactionClient
	 */
	public void  doaudioswtich() {
		printLog("inside doaudioswtich()", LogLevel.MEDIUM);
		if (statusIs(D_CALL)) {
			Message switchreq = MessageFactory.createAduidoSwithcRequest(this);
			doaudioswtich(switchreq);
		}
	}
	/**
	 * @param doaudioswtich
	 *            the doaudioswtich message
	 */
	public void doaudioswtich(Message doaudioswtich) {
		printLog("inside request switch ", LogLevel.MEDIUM);
		if (statusIs(D_CALL)) {
		changeStatus(D_SWITCHING);
		TransactionClient tc = new TransactionClient(sip_provider, doaudioswtich,
				this);
		tc.request();
		}
	}
...
//		MyLog.e("zyf", msg.toString());
		...
		if (msg.isRequest() && msg.isBye()) {
			...
			// switch received
			if (msg.isRequest() && msg.isSwitch()) {

				changeStatus(D_SWITCHING);

				switch_ts = new TransactionServer(sip_provider, msg, this);
				// automatically sends a 200 OK
				Message resp = MessageFactory.createResponse(msg, 210, SipResponses
						.reasonOf(200), null);

				respond(resp);
				listener.onDlgSwitch(this, msg);
			} else
		// cancel received
	...
	public void onTransSuccessResponse(TransactionClient tc, Message msg) {
		...
		if (tc.getTransactionMethod().equals(SipMethods.INVITE)) {
		...
		else if (tc.getTransactionMethod().equals(SipMethods.SWITCH)) {
			Log.e("SWITCH","TC tc.getTransactionMethod equals SWITCH status=" +getStatus());
			if (!verifyStatus(statusIs(D_SWITCHING)))
				return;
			changeStatus(D_CALL);
			StatusLine statusline = msg.getStatusLine();
			int code = statusline.getCode();
			verifyThat(code >= 200 && code < 300, "2xx for bye was expected");
			listener.onDlgSwitchSuccessResponse(this, code,
					statusline.getReason(), msg);
		}
	}
