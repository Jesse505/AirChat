...
public class CallScreen extends Activity implements DialogInterface.OnClickListener {
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case VIDEO_MENU_ITEM:
			...
			try {
				intent = new Intent(this, org.sipdroid.sipua.ui.VideoCamera2.class);
	...
	/*	if (Receiver.call_state == UserAgent.UA_STATE_INCALL && socket == null && Receiver.engine(mContext).getLocalVideo() != 0 && Receiver.engine(mContext).getRemoteVideo() != 0 )
	        (new Thread() {
				public void run() {
					...
					Log.d(TAG,"CallScreen enter video Thread run !!!!");
					try {
					...
					for (;;) {
						try {
							Log.d("RtpSocket","recieve pacaket!!!!!");
					    ...
					    if (videopacket.getPayloadLength() > 2) {
