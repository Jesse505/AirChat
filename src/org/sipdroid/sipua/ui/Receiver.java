...
import com.leadcore.sip.ui.ChatActivity;
import com.leadcore.sip.ui.TabHomeActivity;
import com.leadcore.sip.ui.TabMessageActivity;
...
import com.leadcore.sms.socket.udp.IPMSGConst;
	...
	public class Receiver extends BroadcastReceiver {
		...
		public static final String ACTION_PTT_STATE_CHANGE = "org.sipdroid.sipua.ptt.change";
		public static final String KEY_PTT_STATE = "pttState";
		public static final String KEY_PTT_SOURCE_ADDRESS = "sourceAddress";
		...
		public final static int SMS_NOTIFICATION = 6;
		public final static int SMS_GROUP_NOTIFICATION = 7;
		...
		public static int ptt_state = UserAgent.PTT_STATE_OFF;
		public static String ptt_launcher = "";                  //ptt会话发起方的ip地址
		public static String ptt_group_ip = "";                  //ptt组的地址
		...
		public static String CallType = "calltype"; //add by liuyang
...
//        	context.startService(new Intent(context,RegisterService.class));
		...
		public static void onPttState(int pttState, String sourceAddress){
			MyLog.i("ptt", "Receiver_onPttState oldPttState:" + ptt_state
					+ " newPttState:" + pttState);
			if (ptt_state != pttState) {
				ptt_state = pttState;
				broadcastPttState(ptt_state,sourceAddress);
				switch (ptt_state) {
				case UserAgent.PTT_STATE_ON:
					showPttCallScreen();
//					PttRtpStreamReceiver.speakermode = AudioManager.MODE_NORMAL;
					break;
				case UserAgent.PTT_STATE_OFF:
					
					break;
				case UserAgent.PTT_STATE_FREE:
					MyLog.i("jesse", "onPttState1 PttRtpStreamReceiver.speakermode" +PttRtpStreamReceiver.speakermode);
					PttRtpStreamReceiver.speakermode = speakermode();
					Receiver.engine(mContext).pttSpeaker(AudioManager.MODE_IN_CALL);
					MyLog.i("jesse", "onPttState2 PttRtpStreamReceiver.speakermode" +PttRtpStreamReceiver.speakermode);
					break;
				...
				case UserAgent.PTT_STATE_OCCUPY:
					Receiver.engine(mContext).pttSpeaker(AudioManager.MODE_NORMAL);
					break;
				}
			}
		}
		
		static void broadcastPttState(int state, String sourceAddress){
			MyLog.i("ptt", "Receiver_broadcastPttState pttState:" + state);
			Intent i = new Intent(ACTION_PTT_STATE_CHANGE);
			i.putExtra(KEY_PTT_STATE, state);
			if (sourceAddress != null) {
				i.putExtra(KEY_PTT_SOURCE_ADDRESS, sourceAddress);
			}
		...
		public static void onState(int state,String caller,String type) {
			if (ccCall == null) {
		        ccCall = new Call();
			...
			if (call_state != state) {
				...
				case UserAgent.UA_STATE_INCOMING_CALL:
					...
					//add by lss for declear callist type (video or audio)
					ccConn.setCallType(type);
					//add end
				...
				case UserAgent.UA_STATE_IDLE:
					...
					break;
				case UserAgent.UA_STATE_VIDEO_INCALL:				
	        ...
	        if (text != null) {
				...
				if (type == MISSED_CALL_NOTIFICATION) {
		        	...
		        	case CALL_NOTIFICATION:
		        		notification.contentIntent = PendingIntent.getActivity(mContext, 0, 
		        				createIntent(Activity2.class), 0);
		        		break;
		        	default:
		        		if (type >= REGISTER_NOTIFICATION && mSipdroidEngine != null && type != REGISTER_NOTIFICATION+mSipdroidEngine.pref &&
		        			...
		        			notification.contentIntent = PendingIntent.getActivity(mContext, 0,
		        					createCurrentAcIntent(), 0);
//		        			notification.contentIntent = PendingIntent.getActivity(mContext, 0,
//		        					createIntent(TabHomeActivity.class), 0);
					...
					if (base != 0) {
						...
						if (mSipdroidEngine != null)
							contentView.setTextViewText(R.id.text1, mContext.getResources().getString(R.string.app_name));
...
//								mSipdroidEngine.user_profiles[type-REGISTER_NOTIFICATION].realm_orig);
		...
		public static void showNotification(int type, com.leadcore.sms.entity.Message msg){
			if (type == SMS_NOTIFICATION) {
				// the info to show sms notification
				String textNotification = null;
				Users sender = BaseActivity.mApplication.getOnlineUser(msg.getSenderIMEI());
				String senderName = sender.getNickname();
				switch (msg.getContentType()) {
				case TEXT:
					textNotification =senderName + ":" + msg.getMsgContent();
					break;
				case IMAGE:
					textNotification = senderName + Receiver.mContext.getString(R.string.notification_sms_picture);
					break;
				case VOICE:
	                textNotification = senderName + Receiver.mContext.getString(R.string.notification_sms_voice);
					break;
				case FILE:
					textNotification = senderName + Receiver.mContext.getString(R.string.notification_sms_file);
					break;
				default:
					break;
				}
				showNotification(Receiver.SMS_NOTIFICATION, textNotification, sender);
			}
			else if (type == SMS_GROUP_NOTIFICATION) {
				String textNotification = null;
				String senderName = msg.getSenderName();
				Group group = BaseActivity.mApplication.getOnlineGroup(msg.getReceiveIP());
				switch (msg.getContentType()) {
				case TEXT:
					textNotification =senderName + ":" + msg.getMsgContent();
					break;
				...
				showNotification(Receiver.SMS_GROUP_NOTIFICATION, textNotification, group);
			}
		}
		
		public static void showNotification(int type, String text,Entity entity){
			...
			if (text == null) {
				...
				notification.tickerText = mContext.getString(R.string.notification_sms_ticker);
		        ...
		        contentView.setImageViewResource(R.id.icon, notification.icon);
		        contentView.setTextViewText(R.id.text2, text);
		        contentView.setTextViewText(R.id.text1, mContext.getString(R.string.notification_sms_title));
		        notification.contentView = contentView;
				switch (type) {
				case SMS_GROUP_NOTIFICATION:
			        notification.contentIntent = PendingIntent.getActivity(mContext, 0,
			                createGroupIntent(ChatActivity.class, entity), PendingIntent.FLAG_UPDATE_CURRENT);
			        notificationMgr.notify(type, notification);
					break;
				case SMS_NOTIFICATION:
			        notification.contentIntent = PendingIntent.getActivity(mContext, 0,
			                createUserIntent(ChatActivity.class, entity), PendingIntent.FLAG_UPDATE_CURRENT);
			        notificationMgr.notify(type, notification);
					break;
				default:
					break;
		...
		static void broadcastCallStateChanged(String state,String number) {
			...
			//modify by zyf only system service can send PHONE_STATE broadcast ,so this will throw SecurityException
			try {
				mContext.sendBroadcast(intent, android.Manifest.permission.READ_PHONE_STATE);
			} catch (SecurityException e) {
				MyLog.e("Receiver", "broadcastCallStateChanged",e);
			}
		...
		static Intent createGroupIntent(Class<?>cls,Entity entity) {
        	Intent startActivity = new Intent();
        	startActivity.setClass(mContext,cls);
    	    startActivity.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
    	    startActivity.putExtra(Group.ENTITY_GROUOP, ((Group)entity));
    	    return startActivity;
		}
		
		static Intent createUserIntent(Class<?> cls,Entity entity) {
			Intent startActivity = new Intent();
        	startActivity.setClass(mContext,cls);
    	    startActivity.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
    	    startActivity.putExtra(Users.ENTITY_PEOPLE, ((Users)entity));
    	    return startActivity;
	    ...
	    /**
	     * 
	     * @return 当前栈顶的activity的Intent实例
	     */
	    public static Intent createCurrentAcIntent(){
	    	Intent intent = new Intent(Intent.ACTION_MAIN, null);
	        intent.addCategory(Intent.CATEGORY_LAUNCHER);
	        intent.setComponent(new ComponentName(mContext.getPackageName(), mContext.getPackageName() + "."
	        		+ ActivityCollectorUtils.getLastActivity().getLocalClassName()));
		...
		public static void moveTop(String type) {
			
			Log.d("Sipdroid_UA","calltype=="+type);
			progress();			
        	Intent face_time = new Intent();
        	face_time.setClass(mContext,InCallScreen.class);       	
        	face_time.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        	face_time.putExtra(CallType, type);
			mContext.startActivity(face_time); 
		}
		
		public static void showPttCallScreen(){
			mContext.startActivity(createIntent(PttCallActivity.class));
		}
		...
		public static boolean isFast(int i) {
			if (NetworkUtil.network_type == NetworkUtil.WIFI) {
	        	WifiManager wm = (WifiManager) mContext.getSystemService(Context.WIFI_SERVICE);
...
//		        			return PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.sipdroid.sipua.ui.Settings.PREF_VPN+(i!=0?i:""), org.sipdroid.sipua.ui.Settings.DEFAULT_VPN);  
	        	...
	        	on_wlan = false;
//				return isFastGSM(i);
	        	return false;        //modify by zyf
			}else {
        		on_wlan = true;
        		return true;   
			}
		}
		/**true wifiAp is enable ,otherwise wifiAp is disable**/
